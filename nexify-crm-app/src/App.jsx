...
const emptyLead = { name: "", company: "", country: "", email: "", phone: "", website: "", source: "Cold Outreach", service: "SEO", value: "", stage: "New", category: "", nextFollowUp: "", notes: "" };
...
export default function NexifyCRM() {
  ...
  const saveItem = () => {
    ...
    if (modal === "lead") {
      ...
      if (editId) {
        ...
        newLeads = [{ ...form, id: Date.now(), createdAt: todayStr(), activities: [] }, ...leads];
      }
      setLeads(newLeads);
  ...
  const addTask = (clientId) => {
    ...
    const newClients = clients.map((c) =>
      c.id === clientId ? { ...c, tasks: [...(c.tasks || []), { id: Date.now(), text: taskText.trim(), due: taskDue, done: false }] } : c
    );
    setClients(newClients);
    persist(leads, newClients);
    setTaskText("");
  ...
  const toggleTask = (clientId, taskId) => {
    const newClients = clients.map((c) =>
      c.id === clientId ? { ...c, tasks: (c.tasks || []).map((t) => (t.id === taskId ? { ...t, done: !t.done } : t)) } : c
    );
    setClients(newClients);
    persist(leads, newClients);
  };
  ...
  const filteredLeads = leads.filter((l) => {
    ...
    return matchSearch && matchStage && matchNiche;
  });
